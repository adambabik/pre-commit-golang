...
| Hook ID                  | Description
|--------------------------|------------
| `go-structslop-mod`      | Run `'cd $(mod_root $FILE); structslop -apply [$ARGS] ./...'` for each staged .go file
| `go-structslop-pkg`      | Run `'structslop -apply [$ARGS] ./$(dirname $FILE)'` for each staged .go file
| `go-structslop-repo-mod` | Run `'cd $(mod_root); structslop -apply [$ARGS] ./...'` for each module in the repo
| `go-structslop-repo-pkg` | Run `'structslop -apply [$ARGS] ./...'` in repo root folder
